...
class PostLoginController extends Controller
{
	...
	public function uncompletedQuestionBank(Request $request){	
		...
		// })
		->orderBy('id','DESC')->where('course_id',$request->course_id)->orderBy('id','DESC')->having('otherexam_count', '>', 0)->get();
	...
	public function questionBank(Request $request){	
		$user_id = $request->user()->id;
		$questions = \App\Models\Course::withCount(['question_data as otherexam_count'])->where('status','1')
		// ->whereNotIn('id', function($query) use ($user_id) {
		// 		$query->select('subject_id')
		// 			->from('question_bank_result')
		// 			->whereColumn('question_bank_result.subject_id', 'courses.id')
	...
	public function chapterQuestionBank(Request $request){	
        ...
        $chapter_data = \App\Models\Chapter::where('course_id',$subject_id)->where('status','Active')->orderBy('name','ASC')->get();
		...
		if(!empty($chapter_data)){
			foreach ($chapter_data as $chapter) {
				...
				$topic_data = \App\Models\TopicMaterials::where('course_id',$subject_id)->where('batch_id',$chapter->id)->where('status','1')->orderBy('id','DESC')->get();
				
				$topicResult = [];
				if(!empty($topic_data)){
					foreach($topic_data as $topic){
						$question_count = \App\Models\Question::where('course_id',$subject_id)->where('chapter_id',$chapter->id)->where('topic_id',$topic->id)->count();
						$attempted_question = DB::table('question_bank_answers')->where('subject_id',$subject_id)->where('chapter_id',$chapter->id)->where('topic_id',$topic->id)->where('user_id',$user_id)->count();
						$pause_status = "start";
		...
		return response(["error"=>false,"message" => "Data fetched successfully.","subject_id"=>$subject_data->id,"subject_name"=>$subject_data->name, "result" => $result], 200);
	}
	...
	public function questionBankSearchSubject(Request $request){	
		$title = $request->post('title');
		$subject_data = \App\Models\Course::where('name', 'like', '%' . $title . '%')->where('status','1')->orderBy('id','DESC')->get();


		$subject_data = $questions = \App\Models\Course::withCount(['question_data as otherexam_count'])->where('name', 'like', '%' . $title . '%')->where('status','1')->orderBy('id','DESC')->having('otherexam_count', '>', 0)->get();
	...
	public function questionBankQuestions(Request $request){	
		...
		$attempted_question = DB::table('question_bank_answers')->where(['user_id'=>$user_id,
		'subject_id' => $subject_id,
		'chapter_id' => $chapter_id,
		'topic_id' => $topic_id
		])->count();
		...
		foreach ($questions as $question) {
			...
			$rightPercentage = "";
			$wrongPercentage = "";
			$userAnsData = DB::table('question_bank_answers')->where(['user_id'=>$user_id,
				'subject_id' => $subject_id,
				'chapter_id' => $chapter_id,
				'topic_id' => $topic_id,
	...
	public function previousYearTopics(Request $request){	
		$user_id = $request->user()->id;
        $yearData = DB::table('bulk_question_years')->where('status','1')->orderBy('year','DESC')->groupBy('year')->get();
		...
		if(!empty($yearData)){
			foreach ($yearData as $year) {
				...
				$partData = DB::table('bulk_question_years')->where('status','1')->where('year',$year->year)->get();
				
				$yearResult = [];
				if(!empty($partData)){
					foreach($partData as $part){
						$question_count = DB::table('bulk_questions')->where('year_id',$part->id)->count();
	...
	public function previousYearQuestions(Request $request){
		...
		$questions = DB::table('bulk_questions')->where('year_id',$part_id)->get();
	...
	public function mockTestQuestionList(Request $request){	
		...
		// $mockData = \App\Models\Mockup::select('name')->where('id',$mock_id)->where('course_id',$request->post('course_id'))->first();
	...
	public function subscriptionPlansList(Request $request){	
		...
		$subData = \App\Models\Subscription::orderBy('id','DESC')->where('status','Active')->where('course_id',$request->course_id);
	...
	public function requestPackageSubscription(Request $request){
		...
		try {
			...
			if ($user_id) {
					...
					$callbackUrl = 'https://project.imgglobal.in/edutech/api/webhookResponse';
	...
	public function courseList(Request $request){	
		$title = $request->post('title');
		$course_data = \App\Models\Course::where('name', 'like', '%' . $title . '%')->where('status','1')->orderBy('id','DESC')->get();


		$course_data = $questions = \App\Models\Course::withCount(['question_data as otherexam_count'])->where('name', 'like', '%' . $title . '%')->where('status','1')->orderBy('id','DESC')->having('otherexam_count', '>', 0)->get();
		...
		if(!empty($course_data)){
			foreach ($course_data as $course) {
						$tResult = [
							...
							'questions' => $course->otherexam_count
						];
	...
	public function subjectList(Request $request){	
		...
		$subject_data = \App\Models\Subject::where('name', 'like', '%' . $title . '%')->where('status','1')->where('course_id',$request->course_id)->orderBy('id','DESC')->get();


		$subject_data = $questions = \App\Models\Subject::withCount(['question_data as otherexam_count'])->where('name', 'like', '%' . $title . '%')->where('status','1')->where('course_id',$request->course_id)->orderBy('id','DESC')->having('otherexam_count', '>', 0)->get();
