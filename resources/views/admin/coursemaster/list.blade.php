    ...
    <div class="container-fluid">
                ...
                <div class="card">
                    ...
                    <div class="body">
                        <div class="btn-group top-head-btn">
                            <a class="btn-primary" href="{{ url('admin/coursemaster/add') }}">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="card">
                    <div class="header">
                        <h2><i class="fa fa-th"></i> Course List</h2>
                    </div>
                    <div class="body">
                                                ...
                                                @if(!empty($result))
                                                    @foreach($result as $key => $value)
                                                        <tr class="gradeX odd" id="{{ $value->id }}">
                                                            ...
                                                            <td class="center">
                                                                <a href="{{ url('admin/coursemaster/view/' . $value['id'])}}"
                                                                    title="View Chapters" class="btn btn-tbl-edit">
                                                                    <i class="fas fa-eye"></i>
                                                                </a>
                                                                 @if($permission['write'])
                                                                <a href="{{ url('admin/coursemaster/update/' . $value['id'])}}"
                                                                ...
                                                                <a title="Delete Product"
                                                                    ...
                                                                    href="{{ url('admin/coursemaster/delete/' . $value['id'])}}"
...
@push('custom_js')
    <script>
        $('#course_id').on('change', function () {
            ...
            let url = "{{ route('admin.coursemaster.list') }}?course_id=" + course_id;
        ...
        $('.-change').change(function () {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{ route('admin.coursemaster.changestatus') }}",
                headers: {
    ...
    <script type="text/javascript">
        ...
        function updateOrder(aData) {
            $.ajax({
                url: "{{ route('admin.coursemaster.change-order') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: 'POST',
...
<script>
    $('.-live').change(function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('admin.coursemaster.changelive') }}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
